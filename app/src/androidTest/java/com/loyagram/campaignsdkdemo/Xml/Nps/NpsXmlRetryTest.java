...
public class NpsXmlRetryTest {
    @Rule
    public ActivityTestRule<MainActivity> mActivityTestRule = new ActivityTestRule<>(MainActivity.class);

    @Test
    public void NpsXmlRetryTest() {
        ViewInteraction appCompatButton = onView(
                allOf(ViewMatchers.withId(R.id.fromXml), withText("Show From xml"),
        ...
        onView(withId(R.id.ratingView0)).perform(click());
        onView(withId(R.id.txtReason)).perform(typeText("Test1"), closeSoftKeyboard());
        onView(withId(R.id.btnRetry)).perform(click());
        onView(withId(R.id.ratingView10)).perform(click());
        onView(withId(R.id.txtReason)).perform(typeText("Test2"), closeSoftKeyboard());
        onView(withId(R.id.btnRetry)).perform(click());
        onView(withId(R.id.ratingView9)).perform(click());
        Response response = getResponse();
        if(response.getResponseAnswers().size() == 1) {
            if(response.getResponseAnswers().get(0).getResponseAnswerText().getText().equals("Test1Test2")) {
                Assert.assertTrue(response.getResponseAnswers().get(0).getValue().equals(new BigDecimal(9)));
            }
        } else{
