...
public class NpsXmlRating6Test {

    @Rule
    public ActivityTestRule<MainActivity> mActivityTestRule = new ActivityTestRule<>(MainActivity.class);

    @Test
    public void npsXmlRating6Test() {
        ...
        onView(withId(R.id.ratingView6)).perform(click());
        ...
        if(response.getResponseAnswers().size() == 1) {
            if(response.getResponseAnswers().get(0).getResponseAnswerText().getText().equals("Test")) {
                Assert.assertTrue(response.getResponseAnswers().get(0).getValue().equals(new BigDecimal(6)));
            }
        } else{
